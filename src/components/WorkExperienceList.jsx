...
const WorkExperienceList = () => {
  ...
  // ดึงข้อมูลประสบการณ์ทำงานและผู้ใช้จาก API
  useEffect(() => {
    axios 
      .get("https://localhost:7039/api/Admin/WorkExperiences")
      .then((response) => {
        setWorkExperiences(Array.isArray(response.data) ? response.data : []);
        setLoading(false);
      })
      .catch((error) => {
  ...
  const uniqueUsers = Array.from(
    new Set(filteredExperiences.map((exp) => exp.userID))
  )
      ...
      <div className="flex min-h-screen bg-base-200">
        ...
        <div className="w-1/6 bg-white shadow-lg p-6 rounded-lg">
          ...
          <ul className="menu bg-base-100 text-black rounded-box w-full text-lg">
            ...
            <li><NavLink to="/WorkExperienceList" className={({ isActive }) => isActive ? "hover:bg-gray-300 hover:text-black font-FontNoto font-bold bg-gray-200" : "hover:bg-yellow-100 hover:text-black font-FontNoto font-bold"}>ประสบการณ์ทำงาน</NavLink></li>
        ...
        <div className="flex-1 p-20 bg-white shadow-lg rounded-lg ml-1">
          ...
          <h2 className="text-2xl font-bold text-black font-FontNoto">
            จัดการข้อมูลประสบการณ์ทำงาน
          </h2>
          ...
          <div>
            <span className="font-FontNoto text-lg">จำนวนพนักงาน: {uniqueUsers.length} คน</span>
          </div>
          {loading ? (
                ...
                <tbody>
                  {uniqueUsers.map((user) => (
                    <tr key={user.userID} className="hover:bg-gray-50">
                      ...
                      <td className="border px-4 py-2 font-FontNoto">{user.designation || "ไม่ระบุ"}</td>
                      <td className="border px-4 py-2 text-center font-FontNoto">
                        <button
                          className="btn btn-outline btn-info btn-sm font-FontNoto"
                          onClick={() => navigate(`/WorkExperienceDetail/${user.userID}`)}
                        >
                          ดูรายละเอียด
                        </button>
                      </td>
